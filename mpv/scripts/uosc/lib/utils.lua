...
function load_youtube_heatmap()
	if not state.path or not is_protocol(state.path) then
		return
	end
	-- Match mpv's ytdl whitelist
	if
		not (
			state.path:match("^https?://%w+%.youtube%.com/")
			or state.path:match("^https?://youtube%.com/")
			or state.path:match("^https?://youtu%.be/")
		)
	then
		return
	end
