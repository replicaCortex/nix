...
function create_select_tracklist_type_menu_opener(opts)
	...
	local function serialize_tracklist(tracklist)
		...
		if opts.load_command then
			items[#items + 1] = {
				...
				hint = t("open file"),
				value = "{load}",
				actions = opts.download_command and {
					{ name = "download", icon = "language", label = t("Search online") },
				} or nil,
			}
		end
		if #items > 0 then
	...
	return create_self_updating_menu_opener({
		...
		on_paste = function(event)
			load_track(opts.type, event.value)
		end,
	})
end
...
function open_file_navigation_menu(directory_path, handle_activate, opts)
	...
	local function serialize_items(path, selected_path)
		...
		if is_root then
			if state.platform == "windows" then
				items[#items + 1] =
					{ title = "..", hint = t("Drives"), value = "{drives}", separator = true, is_to_parent = true }
			end
		else
			items[#items + 1] = {
				title = "..",
...
function open_open_file_menu()
	...
	menu = open_file_navigation_menu(directory, function(event)
		if not menu then
			return
		end
		...
		elseif
			itable_has({ nil, "ctrl", "alt", "alt+ctrl" }, event.modifiers)
			and itable_has({ nil, "force_open" }, event.action)
		then
...
function open_subtitle_downloader()
	...
	handle_download = function(data)
		...
		call_ziggy_async(args, function(error, data)
			...
			if should_abort(error, data, function(data)
